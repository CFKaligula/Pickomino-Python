...
def get_closest_lower_tile(num, active_tiles):
    ...
    for tile in active_tiles:
        ...
        if last_tile < num:
            if globals.verbose: print(num, active_tiles)
            result = last_tile
    return result
...
def let_player_roll(rollable_dice_number, player_strategy):        
    ...
    if not set(dice_roll).issubset(set(player_strategy.dice_in_hand)):
        ...
        if globals.verbose: print("dice pick decision:", dice_pick_decision)            
        ...
        if globals.verbose: print("Turn over, no dice to pick...",
              "dice roll:", dice_roll,
...
def player_takes_tile(player_strategy, player_name):
    ...
    if player_dice_sum in stealable_stones_dict:
        if globals.verbose: print(f"tile {player_dice_sum} will be stolen", globals.player_tiles)
        player_name_to_be_stolen_from = stealable_stones_dict[player_dice_sum]
        ...
        if globals.verbose: print("tile is stolen", globals.player_tiles)
    ...
    # 2. add it to the player's tiles
        ...
        if globals.verbose: print("tile to be taken from active:", chosen_tile, "active tiles:", globals.active_tiles)
        globals.active_tiles.remove(chosen_tile)
        globals.player_tiles[player_name].append(chosen_tile)
        if globals.verbose: print("added tile", globals.player_tiles)
           
        
        
def decide_winner(round_num, shuffled_player_list):
    if globals.verbose: print(f"The game is over after {round_num} rounds!")
    ranked_list = {}
    ...
    while len(globals.active_tiles) > 0:
        ...
        for player_name in shuffled_player_list:
            ...
            if ("worm" in player_strategy.dice_in_hand) and player_stopped:
                ...
                if globals.verbose: print("Not allowed to pick tile, no worm or not stopped:",
                     "dice in hand:", player_strategy.dice_in_hand,
                     "stopped:", player_stopped)
                     
                player_loses_turn(player_name)    
                
            if globals.verbose: print(f"player {player_name} turn result:",
                  "dice in hand:",  player_strategy.dice_in_hand,
